...
import {
  ComplianceOnboardingType,
  ComplianceOnboardingVendor,
} from '../../types'
import { dappConfig } from '../../constants/featureConfig'
import { useKeyringStatus, useModal } from '@keyringnetwork/frontend-sdk'
...
export const ComplianceOnboardingButton: React.FC<
  ComplianceOnboardingProps
> = ({ loading, disabled, title, onClick, onSuccess }) => {
  ...
  const doOnboarding = () => {
    ...
    setQuadrataWidget(undefined)
    if (
      dappConfig[chainId].complianceType === ComplianceOnboardingType.SINGLE
    ) {
      if (
        dappConfig[chainId].complianceVendors.includes(
